...
The Feature Proposal Program provides a workflow for activation of Lumos
...
The Lumos validator software supports runtime feature activation through the
...
The
[feature](https://docs.rs/lumos-program/latest/lumos_program/feature/index.html)
and [feature_set](https://docs.rs/lumos-sdk/latest/lumos_sdk/feature_set/index.html)
Rust modules are the primitives for this facility, and the `lumos feature`
command-line subcommands allow for easy feature status inspection and feature
activation.

The `lumos feature activate` workflow was designed for use by the core Lumos
...
### Configuration
The `spl-feature-proposal` configuration is shared with the `lumos` command-line tool.
...
Lumos code base, working with the core Lumos developers at https://github.com/lumos-labs/lumos.
...
```
$ lumos-keygen new --outfile feature-proposal.json --silent --no-passphrase
...
used in the code base and eventually will be visible in the `lumos feature status` command.

Note however that it is not possible to use `lumos feature activate` to
...
After the feature is implemented and deployed to the Lumos cluster,
the *feature id* will be visible in `lumos feature status` and the *feature
...
JSON RPC URL: http://api.mainnet-beta.lumos.com

Distribute the proposal tokens to all validators by running:
    $ lumos-tokens distribute-spl-tokens --from GK55hNft4TGc3Hg4KzbjEmju8VfaNuXK8jQNDTZKcsNF --input-csv feature-proposal.csv --db-path db.8CyUVvio --fee-payer ~/.config/lumos/id.json --owner <FEATURE_PROPOSAL_KEYPAIR>
    $ lumos-tokens spl-token-balances --mint ALvA7Lv9jbo8JFhxqnRpjWWuR3aD12uCb5KBJst4uc3d --input-csv feature-proposal.csv
