...
# Script to setup a local lumos-test-validator with the stake pool program
...
setup_test_validator() {
  ...
  lumos config set --commitment confirmed
  echo "waiting for lumos-test-validator, pid: $pid"
  sleep 15
}
...
create_vote_accounts () {
  ...
  do
    ...
    lumos create-vote-account "$keys_dir/vote_$number.json" "$keys_dir/identity_$number.json" "$keys_dir/withdrawer_$number.json" --commission 1
    vote_pubkey=$(lumos-keygen pubkey "$keys_dir/vote_$number.json")
